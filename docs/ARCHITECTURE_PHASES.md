...
- **Knowledge Base filtering**: `CountryRegulatoryContext.knowledgeSectorMatches` — infers sector from `DashboardCard.niche` via `NicheCatalog` (legacy unmapped niches still treated as medical-device content; empty / `"General"` niches pass sector filter for any profile). `knowledgeNicheMatches` uses `NicheCatalog.resolvePromptKey`; a specific niche chip excludes extra `"General"` rows. `KnowledgeViewModel.filterKnowledge` applies jurisdiction + **sector** + niche chip rules.
- **Seed**: `SeedContentFactory.knowledgeSeedCards(profile, jurisdictionKey)` — full MDR-centric insight/strategy/learning seed for `medical_devices`; sector-agnostic starter modules for other sectors (niche label from first `promptKey`), no synthetic padding. `AppRootViewModel.seedKnowledgeIfNeeded` uses this API.
